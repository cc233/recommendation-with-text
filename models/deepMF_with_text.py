...
class deepMF_with_text(object):
    ...
    def _create_struct(self):
        ...
        #testing
        self.pos_item1=pos_items_embed_lambda*pos_items_embed
        self.pos_item2=pos_items_doc_lambda*pos_items_doc_vec
        self.pos_item3=pos_items_embed
        self.pos_item4=pos_items_doc_vec
    ...
    def _create_loss(self):
        ...
        #testing
        self.test_shape.append(tf.reduce_mean(tf.reduce_sum(self.user_embed*self.pos_item1,axis=1)))
        self.test_shape.append(tf.reduce_mean(tf.reduce_sum(self.user_embed*self.pos_item2,axis=1)))
        self.test_shape.append(tf.reduce_mean(tf.reduce_sum(self.user_embed*self.pos_item3,axis=1)))
        self.test_shape.append(tf.reduce_mean(tf.reduce_sum(self.user_embed*self.pos_item4,axis=1)))
    ...
    def _create_transformed_square_loss(self):
        ...
        self.stop_cu=tf.stop_gradient(self.cu)
        self.stop_bu=tf.stop_gradient(self.bu)
        self.stop_au=tf.stop_gradient(self.au)
        self.loss=1-2*tf.reduce_mean(diff)+2*self.stop_cu*diff-self.stop_cu*self.stop_cu+tf.reduce_mean((pos_scores-self.stop_au)*(pos_scores-self.stop_au))+tf.reduce_mean((self.neg_scores-self.stop_bu)*(self.neg_scores-self.stop_bu))

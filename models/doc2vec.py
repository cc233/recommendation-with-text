...
def calc_doc_vec(doc_index,doc_index_reverse,mask,mask_bool,word_vec,depth,doc_vec_W,doc_vec_b,
                 dim_increase,word_prediction_W,word_prediction_b,word_num,test_shape,layer='doc2vec'):
    ...
    if layer=='word2vec':
        doc_vec=tf.reduce_mean(tf.reduce_mean(word_embed,axis=2),axis=1)
    elif layer=='sent2vec':
        doc_vec=tf.reduce_mean(sent_vec,axis=1)
    #test_shape.append(tf.shape(doc_vec))
